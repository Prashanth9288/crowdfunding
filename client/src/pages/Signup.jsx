...
const Signup = () => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      // Explicitly sign out or just redirect? 
      // User asked: "redirect to login after". 
      // If we are auto-logged in, navigating to login might be weird if login page redirects logged-in users.
      // Assuming standard flow: Account created -> Please Login.
      toast({
    ...
    <div className="h-screen w-screen bg-[#F8F7FA] flex items-center justify-center relative overflow-hidden">
      ...
      <div className="bg-white rounded-[32px] w-full max-w-[480px] p-8 md:p-12 shadow-xl shadow-slate-200/40 relative z-10 animate-fade-in-up">
        ...
        <div className="flex flex-col items-center text-center mb-8">
            ...
            <h2 className="text-2xl md:text-3xl font-bold text-slate-900 mb-2">Create your account</h2>
            <p className="text-slate-500 text-sm md:text-base">Join thousands of creators and backers on FundFlow</p>
        </div>

        <form onSubmit={handleSignup} className="space-y-5">
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-700 block" htmlFor="fullName">Full Name</label>
        ...
        <div className="text-center text-sm mt-8 text-slate-500 font-medium">
            ...
            <Link to="/auth/login" className="font-bold text-mint-600 hover:text-purple-600 transition-colors">
            Sign in
            </Link>
        </div>
      </div>
